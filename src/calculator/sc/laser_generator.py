...
class LaserGenerator(ConstInitializer):
    ...
    def _generate_ele(self, t: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        ele_x = amp * np.cos(self.omega_0 * t)
        ele_y = np.zeros(t.size, dtype=np.float64)
        return ele_y, ele_x
    ...
    def _generate_vec_pot(self, t: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # vec_pot_y
        vec_pot_y = np.zeros(t.size, dtype=np.float64)
    ...
    def save_laser_info(self):
        ...
        my_df.to_hdf(self.sc_laser_data_file, key=self.laser_name)
