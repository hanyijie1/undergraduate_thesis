...
def _assist_integrate_spectrum(
    ...
    p_count, theta_count, alpha_count = info_arr.shape
    integrate_info_matrix = np.zeros(
        (p_count, alpha_count),
        dtype=info_arr.dtype
    )
    for alpha_id in range(alpha_count):
        for p_point in range(p_count):
            info_vec = info_arr[p_point, :, alpha_id]
            ...
            integrate_info_matrix[p_point, alpha_id] = integrate_info_value
