...
class TransProSpectVisualizer(ConstInitializer):
    ...
    def __init__(self):
        super().__init__()
        # data input
        with h5py.File(self.sc_sfa_data_file, 'r') as f:
    ...
    def plot_trans_pro_spect(self, ):
        ...
        plt.savefig(f"{self.sc_sfa_trans_pro_spect_graph_path}.png", format='png', dpi=1000, bbox_inches='tight')
        plt.savefig(f"{self.sc_sfa_trans_pro_spect_graph_path}.pdf", format='pdf', bbox_inches='tight')
        plt.show()