...
class TransProSpectVisualizer(ConstInitializer):
    ...
    def __init__(self):
        super().__init__()
        # data input
        with h5py.File(self.otc_cva_data_file, 'r') as f:
    ...
    def plot_trans_pro_spect(self, ):
        ...
        for i in range(self.visual_count):
            max_visual_x = 0.2
            max_visual_y = np.percentile(self.spectrum_matrix[:, i], 97)
        ...
        plt.savefig(f"{self.otc_cva_trans_pro_spect_graph_path}.png", format='png', dpi=1000, bbox_inches='tight')
        plt.savefig(f"{self.otc_cva_trans_pro_spect_graph_path}.pdf", format='pdf', bbox_inches='tight')
        plt.show()