...
class LaserVisualizer(ConstInitializer):
    ...
    def plot_laser(self):
        # get datas from HDF5
        my_df = pd.read_hdf(self.otc_laser_data_file, key=self.laser_name)
        # distribution
        ...
        plt.savefig(f"{self.otc_laser_graph_path}.png", format="png", dpi=1000, bbox_inches='tight')
        plt.savefig(f"{self.otc_laser_graph_path}.pdf", format="pdf", bbox_inches='tight')
        plt.show()